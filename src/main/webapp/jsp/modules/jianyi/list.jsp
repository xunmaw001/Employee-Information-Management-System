...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">建议管理</li>
                            <li class="breadcrumb-item active">建议列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        ...
                                        <div class="layui-input-inline">
                                            <input type="text" id="jianyiNameSearch" style="width: 140px;" class="form-control form-control-sm"
                                                   placeholder="建议名称" aria-controls="tableId">
                                        </div>
                                    </div>
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        建议类型
                                        <div class="layui-input-inline">
                                            <select name="jianyiTypesSelectSearch"  style="width: 150px;" id="jianyiTypesSelectSearch" class="form-control form-control-sm"
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        职位
                                        <div class="layui-input-inline">
                                            <select name="zhiweiTypesSelectSearch"  style="width: 150px;" id="zhiweiTypesSelectSearch" class="form-control form-control-sm"
                                                    aria-controls="tableId">
                                            </select>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        <!-- 级联表的级联字典表 -->
            ...
            //建议名称
            var jianyiNameSearchInput = $('#jianyiNameSearch');
            if( jianyiNameSearchInput != null){
                if (jianyiNameSearchInput.val() != null && jianyiNameSearchInput.val() != '') {
                    searchForm.jianyiName = $('#jianyiNameSearch').val();
                }
            }
     
                //建议类型
            var jianyiTypesSelectSearchInput = document.getElementById("jianyiTypesSelectSearch");
            if(jianyiTypesSelectSearchInput != null){
                var jianyiTypesIndex = jianyiTypesSelectSearchInput.selectedIndex;
                if( jianyiTypesIndex  != 0){
                    searchForm.jianyiTypes= document.getElementById("jianyiTypesSelectSearch").options[jianyiTypesIndex].value;
                }
            }
                        getDataList();
        }
        ...
        function getDataList() {
            http("jianyi/page", "GET", {
                page: pageIndex,
                limit: pageSize,
                sort: sortColumn,
                order: sortOrder,
                //本表的
                jianyiName: searchForm.jianyiName,
                jianyiTypes: searchForm.jianyiTypes,
            //级联表的
        ...
        function setDataRow(item, number) {
            ...
            //建议名称
            var jianyiNameCell = document.createElement('td');
            jianyiNameCell.innerHTML = item.jianyiName;
            row.appendChild(jianyiNameCell);


            //建议类型
            var jianyiTypesCell = document.createElement('td');
            jianyiTypesCell.innerHTML = item.jianyiValue;
            row.appendChild(jianyiTypesCell);
            ...
            row.appendChild(insertTimeCell);
                ...
                //建议详情
            var jianyiContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.jianyiContent && item.jianyiContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.jianyiContent.indexOf('<img');
                var endIndex = item.jianyiContent.indexOf('>');
                    jianyiContentCell.innerHTML = item.jianyiContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.jianyiContent != null && item.jianyiContent != "" && item.jianyiContent.length >= 11) {
                        jianyiContentCell.innerHTML = item.jianyiContent.substring(0, 10) + "...";
                } else {
                        jianyiContentCell.innerHTML = item.jianyiContent;
                }
            }
            row.appendChild(jianyiContentCell);
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("jianyi/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addjianyi", "addjianyi");
            window.location.href = "add-or-update.jsp"
        }
    ...
    //填充本表搜索下拉框
        ...
        function jianyiTypesSelectSearch() {
            var jianyiTypesSelectSearch = document.getElementById('jianyiTypesSelectSearch');
            if(jianyiTypesSelectSearch != null) {
                jianyiTypesSelectSearch.add(new Option('-请选择-',''));
                if (jianyiTypesOptions != null && jianyiTypesOptions.length > 0){
                    for (var i = 0; i < jianyiTypesOptions.length; i++) {
                            jianyiTypesSelectSearch.add(new Option(jianyiTypesOptions[i].indexName,jianyiTypesOptions[i].codeIndex));
                    }
                }
            }
        }
    ...
    //查询当前表搜索条件所有列表
            function jianyiTypesSelect() {
                //填充下拉框选项
                http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jianyi_types", "GET", {}, (res) => {
                    if(res.code == 0){
                        jianyiTypesOptions = res.data.list;
                    }
                });
            }
